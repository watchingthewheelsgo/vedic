...
6. **少修辞**：只有在能帮助理解时才使用比喻；禁止为了"有文采"牺牲清晰度
...
3. **信息完成标准**：不再用固定字数衡量质量。每个审计/板块必须完成"结论→证据→限制→行动含义"四项；够清楚就停，禁止为凑长度重复术语或换说法。
...
检查prevalidation_result.json是否存在：
  → 存在 → 读取 score / decision / timeConfidence，作为验前事和时间置信度唯一结构化来源
  → 不存在 → 从structured_data.md读取旧版验前事摘要，并在报告中标注"未发现结构化验前事结果"

检查birth_input_context.json和sensitivity_scan.json是否存在：
  → 存在 → 读取 summary.riskLevel / changedFields / divisionalConfidence，作为输入风险和分盘可信度唯一结构化来源
  → 不存在 → 从structured_data.md的"输入可信度与敏感度"和"分盘可信度声明"读取；仍不存在则标注"未发现输入敏感度扫描"
```
...
> 时间置信度：[从prevalidation_result.json decision.timeConfidence读取，如存在]
> 输入风险：[从sensitivity_scan.json summary.riskLevel读取，如存在]
```

若 `sensitivity_scan.summary.riskLevel=high` 且 `prevalidation_result.decision` 未明确通过，
不得继续生成强确定性完整报告；先要求运行/补完验前事校正。
任何 `divisionalConfidence` 为 low 的分盘只能作为弱参考，不得作为核心结论锚点。
必须执行 `sensitivity_scan.reportReadiness.llmContract`：
- `mustNotUseAsPrimaryEvidence` 中的字段不得作为主结论支点。
- `mayUseAsPrimaryEvidence` 才能作为主线证据。
- `reportReadiness.mode=rectification_required` 时，不生成完整 Step 1-5 报告；只输出需要校正的说明，除非 backend 的 `prevalidation_result.json.decision.reportAllowed=true`。
- `decision.reportScope=guarded_full_report` 时，每个涉及分盘/时序的段落必须显式标注置信度，低可信分盘只放 appendix 或限制说明。
...
主报告禁止写成参数堆叠。P1、SAV、L10、AmK、Dasha等术语可以保留，但必须在第一次出现时用括号翻译；后文只在证据句中使用。
...
- 吠陀视相关系（同座接触 + Graha Drishti；度数只作强弱标注）
- 宫主表
- SAV/BAV数据
- Nakshatra
- Chara Karakas
...
B级信号（正常篇幅）：
  - 入庙(Own Sign)的行星
  - 逆行的行星
  - 有紧密同座接触或 Graha Drishti 度数强弱标注为"紧密"的行星
...
2. 逐条检查当前盘是否满足：
   ...
   - Dhana Yoga（2宫主+11宫主互动）
   - Raja Yoga（三角宫主+角宫主合相/互视/互溶）
   - VRY（6/8/12宫主互溶或合相于凶宫）
...
```
写法要求：
  - 像一次专业咨询：判断清楚、证据明确、建议具体
  - 每板块按"是什么→为什么→限制/置信度→怎么办"组织；不强制3-5段
  - 术语必须翻译："你的10宫主（管事业的那颗星）"，不是"L10"
  - 引用数据时自然嵌入："那颗星力量全盘第一（181%），状态很好"
  - 给具体的、用户能操作的建议，不说"注意平衡"
  - 不使用表演式口吻；比喻只能用于澄清，不用于装饰
  - 涉及痛点时直说影响、证据和可控边界，不用安慰性套话覆盖风险
...
禁止：
  ...
  - 模板化的开头/结尾
  - 花哨但不提供信息的词：先天加持、灵魂课题、命运召唤、宇宙安排、喝茶聊天、掰开揉碎
...
板块4: 感情/婚姻 — 何时遇到对的人
  ...
  → 配偶画像：L7+Venus+DK三角交叉（DK=7K体系确定值，直接使用）
  → 配偶来源线索：UL所在宫位/星座
  → 感情的最大挑战和最大优势
  → 关键窗口期
...
板块9: 长期成长 — 价值观与内在驱动
  ...
  → 长期价值观、信念系统、学习/修行方式
  → 写得深，但必须落在可理解的行为模式上，不写玄化口号

板块10: 赛道优势地图 — 哪些方向更容易形成复利
  引用: p2a~p2d(全部格局), p3a(D9结算汇总), appendix(Dasha速查表)
  → 参考: resources/yogas.md
  → 格局扫描但不列格局名单，翻译成"哪些能力/赛道更容易持续兑现"
...
写入 `report_quality_audit.md`。这是最后一道质量门，不重写正文，但必须指出报告是否可交付，以及需要修正的具体文件/段落。

读取：
- `prevalidation_result.json`（如存在）
- `birth_input_context.json`（如存在）
- `sensitivity_scan.json`（如存在）
- `structured_data.md`
- `p5a_life.md`
- `p5b_life.md`
- `appendix.md`
...
|------|--------|----------|-----|
| Time confidence used consistently | PASS/FAIL | ... | ... |
| Input sensitivity contract obeyed | PASS/FAIL | ... | ... |
| Restricted evidence not used as primary conclusion | PASS/FAIL | ... | ... |
| Missed validation anchors not reinterpreted as hits | PASS/FAIL | ... | ... |
| Child/adult/elder framing consistent | PASS/FAIL | ... | ... |
| Dasha ages and date windows coherent | PASS/FAIL | ... | ... |
| Main report explains terms before using labels | PASS/FAIL | ... | ... |
| No two consecutive sentences are pure technical data | PASS/FAIL | ... | ... |
| Appendix keeps technical detail out of the main report | PASS/FAIL | ... | ... |
| No medical/legal/financial deterministic claims | PASS/FAIL | ... | ... |

## Required Revisions
- [file]: [specific section/paragraph] → [what to change]
```

硬规则：
- 如果 `prevalidation_result.json.decision.reportAllowed=false`，Verdict 必须是 NEEDS_REVISION，除非报告明确写成 low-confidence / D1-only。
- 如果 `sensitivity_scan.reportReadiness.llmContract.mustNotUseAsPrimaryEvidence` 中的字段被用作主结论支点，Verdict 必须是 NEEDS_REVISION。
- 如果 `sensitivity_scan.reportReadiness.mode=rectification_required` 但正文仍写成强确定性完整报告，Verdict 必须是 NEEDS_REVISION。
- 未成年盘主如果出现当下婚恋、创业、投资、财富决策建议，Verdict 必须是 NEEDS_REVISION。
- 如果 Dasha 年龄和出生日期不一致，Verdict 必须是 NEEDS_REVISION。
- 如果报告把不准的验前事改口解释成命中，Verdict 必须是 NEEDS_REVISION。
...
工作目录/
  ...
  appendix.md         ← Step 5
  report_quality_audit.md ← Step 6
```
...
已生成：p2a ~ p5b + appendix + report_quality_audit（共12个文件）
...
当用户说"生成报告""打包""导出HTML"时，请求 backend report export/tool。
支持`--include`参数选择性打包。
...
7. **代价必提**：带偏置信号必须明确副作用
8. **人话优先**：70%解读+20%数据+10%注释；主报告不堆术语，技术细节进入 appendix
9. **事件关联是陈述**：嵌入宫位分析，不单独设验证环节
10. **分盘可信度**：⚠️标注的分盘，分析措辞留余地
11. **格局不定命**：说赛道+方向+时间，不评高低
12. **反确认偏误**：不得从用户经历反推盘面含义，Dasha必须双向分析
13. **最终质检**：主报告完成前检查每个板块是否有清楚结论、证据、限制和行动含义；连续两句纯技术描述必须改写或移到附录
