...
🔴 关键数据（缺一不可，缺少则停下要求补充）：
  ...
  □ Ayanamsa：用的什么岁差体系（本系统以Lahiri为主计算口径，并用True Chitrapaksha做边界交叉校验）
    ⚠️ 非Lahiri/Chitrapaksha系岁差（KP/Raman/Pushya等）会导致度数偏差，影响分析准确度
...
```
在数据源中搜索以下关键词：
  "Lahiri" / "Chitrapaksha"     → ✅ 与本系统主口径一致，继续
  "True Chitrapaksha" / "True Chitra" → ✅ 可作为交叉校验口径；若Lagna边界敏感，需标注风险
  "KP" / "Krishnamurti"        → ⚠️ 警告：检测到KP岁差
  "Raman"                      → ⚠️ 警告：检测到Raman岁差
  "Pushya" / "Pushyapaksha"    → ⚠️ 警告：与本系统差约1.14°，运势时间线会偏移
  未检测到任何标注              → 记录"未检测到"，不阻塞

检测到非Lahiri/Chitrapaksha系时（KP/Raman/Pushya）：
  1. structured_data标注 ⚠️「Ayanamsa风险：检测到[X]，非Lahiri主口径」
  2. 告知用户："建议使用Lahiri岁差重新导出，或使用本系统calculator重算，否则分析可能存在偏差"
...
用预分析结果做3-5条**用户可直接回答的生活事实问题**。它的目的不是制造惊艳感，而是用可证伪事实验证出生时间、候选区间和信号选择是否可靠。

**用户可见文案硬约束**：
- 编号后只写一个普通人能立即理解、并能回答“准/部分准/不准”的问题，使用会话语言。
- 一张卡只验证一件事；中文尽量不超过45个字，英文尽量不超过35个词。
- 优先询问已发生、有时间范围的重大事件；否则询问具体的家庭、教育、搬迁、职业或关系事实。
- 可见问题禁止出现行星、星座、宫位、度数、Yoga、Nakshatra、Dasha、梵文术语、候选编号或评分。这些只能放在引用块的推导行中。
- 禁止讨好式性格概括、多事件打包和引导性推断。
- 对 minor 不得询问成年人的婚姻、职业或生育经历；必须改用已发生的家庭、成长或健康照护事实。
...
**1.** [用普通语言询问一个结构性事实，以？结尾]

> 推导：[简要数据来源，如"L9=Saturn入庙在9宫"]

**2.** [用普通语言询问另一个生活事实，以？结尾]

> 推导：[简要数据来源]

**3.** 在[YYYY-YYYY]年期间，您是否经历过[一个具体事件]？
...
⚠️ 排版硬规则（不可压缩）：
- 用户问题用 `**1.**` 加粗编号，普通文本，必须以问号结尾
- 推导标注用 `>` 引用块，与推断正文之间空一行
- 每条推断之间空一行分隔
- 正确排版示例（实际输出效果）：

**1.** 成长过程中，父亲是否长期主导家里的重要决定？

> 推导：L9=Saturn入庙在9宫自己家

**2.** 您的学业是否曾有一段明显吃力或中断的阶段？

> 推导：L5=Mercury燃烧(距Sun 2.46°) → 智慧受压制

**3.** 2016到2018年期间，您是否完成过一次重要的升学或资格获得？
