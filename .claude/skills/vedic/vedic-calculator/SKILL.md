...
2. 生成 `birth_input_context.json`：记录用户原始时间/地点、解析后的经纬度、timezone、地点来源、地点精度、允许反向校正的边界。
3. 生成 `sensitivity_scan.json`：按时间精度和地点半径做小样本扰动扫描，标记 Lagna/Moon Nakshatra/分盘/Dasha 是否处在边界；同时输出 `candidateGroups`、`stability`、`reportReadiness` 和 `llmContract`。
4. 使用原 formatter 契约生成 `structured_data.md`，并在开头写入"输入可信度与敏感度"和动态分盘可信度。
5. 校验 SAV 总计 = 337。
...
3. Ra-Ke差180° ✅
4. Lagna星座是否合理
5. `birth_input_context.json` 是否存在，且地点 source/accuracy/radiusKm 不为空
6. `sensitivity_scan.json` 是否存在，且 summary.riskLevel 与 structured_data 的分盘可信度一致
7. `sensitivity_scan.reportReadiness.llmContract.mustNotUseAsPrimaryEvidence` 是否列出所有不稳定字段

### Step 5: 模式选择

structured_data.md / birth_input_context.json / sensitivity_scan.json 生成后，向用户输出：

```
✅ 排盘完成！所有数据已生成（行星/分盘/SAV/Dasha+小运/宫主表/尊贵度/过运…）
✅ 已完成输入敏感度扫描（时间偏移/地点半径/分盘边界）
...
- 用户选 a) 或说"直接分析"/"开始" → 触发 vedic-reader（精简模式：跳过提取，直接读 structured_data + sensitivity_scan → 验前事/校正）
...
chart = {
    # 基础天文
    'ayanamsa': 23.8982,           # float, Lahiri ayanamsa 度数（主计算口径）
    'lagna': {
