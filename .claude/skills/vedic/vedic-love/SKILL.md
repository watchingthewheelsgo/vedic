...
5. **少修辞**：只有在帮助理解关系模式时才使用比喻；禁止为了文采牺牲清晰度
...
3. **信息完成标准**：不再用固定字数衡量质量。每个Step必须完成"关系结论→证据→限制/风险→行动建议"四项；够清楚就停，禁止为凑长度重复术语或换说法。
...
每个Step写完前检查：
- 主段落先说关系含义，再给 DK/UL/PK/5宫/7宫/D9/Dasha 证据。
- 裸术语必须翻译：DK=配偶指示星，UL=婚姻呈现点，D9=关系承载盘。
- 时间窗口必须引用 Dasha 或 D9/D1 交叉证据，不凭感觉给年份。
- 未成年盘主不写恋爱、婚姻、配偶时机；只写关系边界、情感表达和长期亲密能力培养。
- **老年盘主（elder_subject）感情话题降级**：不主动推演"晚年是否孤独/伴侣先离世"这类议题；
  Maraka/8宫/12宫相关信号在感情分析中一律不主动提及，用户主动问"另一半的健康/我们谁先走"
  这类问题时，明确告知"这类问题不适合用星盘技术回答，建议关注实际健康管理"，不强行给出盘面推导。
- 禁止花哨但不提供信息的词：正缘降临、命运安排、宇宙安排、灵魂伴侣召唤、喝茶聊天、掰开揉碎。
...
- 对5宫的视相 → 从吠陀视相关系/宫位被视相表读取

**7宫(伴侣宫)审计:**
- 7宫主星落点 → 从宫主表读取
- 7宫内星 → 伴侣特质
- 对7宫的视相 → 从吠陀视相关系/宫位被视相表读取
...
感情时间窗口的判断必须引用 `.claude/skills/vedic/vedic-core/resources/house_framework.md` 的Dasha硬约束：
- 对每个大运/小运执行正面条件/负面条件判定
- 禁止美化凶宫主大运为"灵魂功课"
- **距当前日期>10年的窗口，必须按house_framework「预测时间距离置信度」降级颗粒度**
- 时间节点必须带括号标注推导依据：
