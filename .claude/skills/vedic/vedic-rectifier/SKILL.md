...
- 先用逻辑判断，不够再上计算工具
- 区分"候选区间"和"精确校时"：轻量边界矫正只能缩小范围；只有5个重大事件匹配足够时，才给出±5分钟结论
- 不把候选范围包装成确定出生时间；置信度不足时必须保留范围和限制
...
如果工作目录存在 `prevalidation_boundary_correction.md`：
```
→ 先读取它
→ 使用其中的推荐时间区间作为本次rectifier的初始扫描范围
→ 不重复询问候选边界问题
→ 如果候选区间仍无法缩小到±5分钟，再进入Step 2收集5个重大人生事件
```

注意：`prevalidation_boundary_correction.md` 是验前事失败后的轻量边界矫正，
只能缩小候选区间，不能代替完整事件校时。完整rectifier仍以重大人生事件+Dasha匹配为准。
如果用户只完成了边界矫正、没有提供足够重大事件，本skill只能输出"推荐区间 + 置信度 + 需要哪些事件继续校准"，不能输出精确分钟。
