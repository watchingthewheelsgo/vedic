...
class Chart:
    ...
    def _parse(self):
        ...
        for m in re.finditer(
            r"^\|\s*(\d{1,2})\s*\|\s*[^|]+\|\s*([A-Za-z]+)\s*\|\s*(\d+)\s*\|", t, re.M
        ):
            h = int(m.group(1))
        ...
        rows.append(
            {
                ...
                "sign": p["sign"],
                "house_in_dst": h,
                "dignity": src.dignity_of(name),
                "roles": src.role_of(name),
            }
        )
    return rows
    ...
    for an in PLANETS:
        ...
        for pt, psidx in dst_points.items():
            if psidx in target_signs:
                ...
                hits.append({"a": an, "point": pt, "nature": nature, "dignity": src.dignity_of(an)})
    return hits
...
def dasha_overlap(a, b):
    ...
    if a.dasha_md:
        ...
        lines.append(
            f"{a.label}: MD {a.dasha_md['planet']} ({a.dasha_md['start']}~{a.dasha_md['end']}){ad}"
        )
    if b.dasha_md:
        ad = f" / AD {b.dasha_ad['planet']}" if b.dasha_ad else ""
        lines.append(
            f"{b.label}: MD {b.dasha_md['planet']} ({b.dasha_md['start']}~{b.dasha_md['end']}){ad}"
        )
...
def build(a, b):
    ...
    for r in overlay(a, b):
        L.append(
            f"| {r['planet']} | {r['sign']} | {r['house_in_dst']} | {r['dignity']} | {r['roles'] or '—'} |"
        )
    L.append("")
    ...
    L.append("## 8. 置信度\n")
    L.append(
        f"- A 行星解析: {len([p for p in PLANETS if p in a.planets])}/9 | "
        f"B 行星解析: {len([p for p in PLANETS if p in b.planets])}/9"
    )
    L.append(
        f"- A Moon宿: {'有' if a.moon_nak else '缺'} | B Moon宿: {'有' if b.moon_nak else '缺'}"
    )
    L.append(
        "- 解读时以 D1 方向叠盘 > UL·DK > D9 > Dasha > 月宿 为权重（interpretation-rubric.md）"
    )
    return "\n".join(L)
...
def main():
    ...
    print(f"[ok] {out}")
    print(
        f"  A={args.a} planets {len([p for p in PLANETS if p in a.planets])}/9, "
