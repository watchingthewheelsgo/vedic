...
def format_structured_data(chart, transit_data, meta, user_info):
    ...
    if sp:
        ...
        if "AL" in sp:
            lines.append(
                f"| AL (Arudha Lagna) | {sp['AL']['sign']} | {sp['AL']['house']} | 外在形象/世人眼中的你 |"
            )
        if "UL" in sp:
            lines.append(
                f"| UL (Upapada Lagna) | {sp['UL']['sign']} | {sp['UL']['house']} | 婚姻/伴侣宫 |"
            )
        lines.append("")
    ...
    for d in chart["dashas"]:
        if "antardashas" in d:
            ...
            for ad in d["antardashas"]:
                ...
                lines.append(
                    f"| {d['planet']}-{ad['planet']} | {ad['start']} | {ad['end']} |{marker}"
                )
            lines.append("")
    ...
    if current_dasha:
        ...
        lines.append("```")
        lines.append(
            f"Mahadasha: {current_dasha['planet']} ({current_dasha['start']} ~ {current_dasha['end']})"
        )
        ...
        if current_ad_info:
            lines.append(
                f"Antardasha: {current_dasha['planet']}-{current_ad_info['planet']} ({current_ad_info['start']} ~ {current_ad_info['end']})"
            )
        lines.append("```\n")
    ...
    for a in chart["aspects"]:
        ...
        gap_text = (
            f"{gap}° / {a.get('strength', '整宫')}"
            if gap is not None
            else a.get("strength", "整宫")
        )
        lines.append(
