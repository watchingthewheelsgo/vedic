...
def calculate_shadbala_fixed(year, month, day, hour, minute, lat, lon, tz_offset):
    ...
    for dcf in sv_factors:
        if dcf != 2:
            pp_sv[dcf] = charts.divisional_chart(jd, place, divisional_chart_factor=dcf)[
    ...
    # ========== TOTAL ==========
    totals = [
        sthana[i] + kaala[i] + dig[i] + cheshta[i] + naisargika[i] + drik_b[i] for i in range(7)
    ]
