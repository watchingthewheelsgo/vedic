    ...
    try:
        ...
        if isinstance(bb, (list, tuple)):
            ...
            if isinstance(bb[0], (list, tuple)):
                ...
                for h in range(12):
                    result[h + 1] = {
                        "total": round(bb[-1][h], 2)
                        if len(bb) > 3
                        else round(sum(b[h] for b in bb), 2),
                    }
...
def calculate_vargeeya_bala(year, month, day, hour, minute, lat, lon, tz_offset):
    ...
    try:
        ...
        if isinstance(pvb, dict):
            result["pancha_vargeeya"] = {PLANETS[k]: round(v, 2) for k, v in pvb.items() if k < 7}
    ...
    try:
        ...
        if isinstance(dvb, dict):
            result["dwadhasa_vargeeya"] = {PLANETS[k]: round(v, 2) for k, v in dvb.items() if k < 7}
